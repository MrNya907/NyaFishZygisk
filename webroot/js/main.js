...
(async () => {
  ...
  const nyafishzygisk_state = document.getElementById('nyafishzygisk_state')
  const nyafishzygisk_icon_state = document.getElementById('nyafishzygisk_icon_state')
  ...
  console.log('[nyafishzygisk.js] Android version: ', androidVersionCmd.stdout)
  ...
  console.log('[nyafishzygisk.js] Kernel version: ', unameCmd.stdout)

  const catCmd = await exec('/system/bin/cat /data/adb/nyafishzygisk/module.prop')
  console.log(`[nyafishzygisk.js] nyafishzygisk module infomation:\n${catCmd.stdout}`)

  let expectedWorking = 0
  let actuallyWorking = 0

  const nyafishzygiskInfo = {
    rootImpl: null,
    monitor: null,
    zygotes: [],
    daemons: []
  ...
  if (catCmd.errno === 0) {
    ...
    moduleInfo.match(/\(([^)]+)\)/g).forEach((area) => {
      ...
      nyafishzygiskInfo.rootImpl = rootImpl
    ...
    infoArea.forEach((info) => {
      if (info.startsWith('monitor:')) {
        nyafishzygiskInfo.monitor = info.substring('monitor: X '.length).trim()
      }

      if (info.startsWith('zygote')) {
        nyafishzygiskInfo.zygotes.push({
      ...
      if (info.startsWith('daemon')) {
        nyafishzygiskInfo.daemons.push({
          bits: info.substring('daemon'.length, 'daemon'.length + 'XX'.length),
          state: info.substring('daemonXX: X '.length).trim(),
          modules: daemonModules[nyafishzygiskInfo.daemons.length] || []
        })
      }
    })

    switch (nyafishzygiskInfo.monitor) {
    ...
    expectedWorking = nyafishzygiskInfo.zygotes.length

    for (let i = 0; i < nyafishzygiskInfo.zygotes.length; i++) {
      const zygote = nyafishzygiskInfo.zygotes[i]
      /* INFO: Not used ATM */
      /* const daemon = nyafishzygiskInfo.daemons[i] */
  ...
  if (expectedWorking === 0 || actuallyWorking === 0) {
    nyafishzygisk_state.innerHTML = translations.page.home.status.notWorking
  } else if (expectedWorking === actuallyWorking) {
    nyafishzygisk_state.innerHTML = translations.page.home.status.ok

    rootCss.style.setProperty('--bright', '#3a4857')
    nyafishzygisk_icon_state.innerHTML = '<img class="brightc" src="assets/tick.svg">'
  } else {
    nyafishzygisk_state.innerHTML = translations.page.home.status.partially

    rootCss.style.setProperty('--bright', '#766000')
    nyafishzygisk_icon_state.innerHTML = '<img class="brightc" src="assets/warn.svg">'
  }

  if (nyafishzygiskInfo.rootImpl)
    root_impl.innerHTML = nyafishzygiskInfo.rootImpl

  const all_modules = []
  nyafishzygiskInfo.daemons.forEach((daemon) => {
  ...
  if (all_modules.length !== 0) {
    ...
    console.log(`[nyafishzygisk.js] Module list:`)
  ...
  console.log('[nyafishzygisk.js] boot time: ', Date.now() - start_time, 'ms')
