...
/* INFO: nyafishzygisk already performs a fork in ZygiskContext::fork_pre, because of that,
...
bool update_mnt_ns(enum mount_namespace_state mns_state, bool dry_run) {
    ...
    if (nyafishzygiskd_update_mns(mns_state, ns_path, sizeof(ns_path)) == false) {
...
int ZygiskModule::connectCompanion() const {
    return nyafishzygiskd_connect_companion(id);
}

/* Zygisksu changed: Use own zygiskd */
int ZygiskModule::getModuleDir() const {
    return nyafishzygiskd_get_module_dir(id);
}
...
bool ZygiskContext::load_modules_only() {
  ...
  if (nyafishzygiskd_read_modules(&ms) == false) {
...
void ZygiskContext::app_specialize_pre() {
    ...
    /* INFO: Isolated services have different UIDs than the main apps. Because
               ...
               app, we need to ensure that the UID sent to nyafishzygiskd to search
    ...
    */
    ...
    info_flags = nyafishzygiskd_get_process_flags(uid, (const char *const)process);
...
void ZygiskContext::nativeForkSystemServer_pre() {
    ...
    load_modules_only();
    run_modules_pre();
    nyafishzygiskd_system_server_started();
