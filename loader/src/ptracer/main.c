...
int main(int argc, char **argv) {
  printf("The nyafishzygisk Tracer %s\n\n", ZKSU_VERSION);
  ...
  if (argc >= 2 && strcmp(argv[1], "monitor") == 0) {
    ...
    return 0;
      ...
      if (argc >= 4 && strcmp(argv[3], "--restart") == 0) nyafishzygiskd_zygote_restart();
    ...
    enum nyafishzygiskd_command command;

    if (strcmp(argv[2], "start") == 0) command = START;
    else if (strcmp(argv[2], "stop") == 0) command = STOP;
    else if (strcmp(argv[2], "exit") == 0) command = EXIT;
    else {
      printf("[nyafishzygisk]: Usage: %s ctl <start|stop|exit>\n", argv[0]);

      return 1;
    }

    if (send_control_command(command) == -1) {
      printf("[nyafishzygisk]: Failed to send the command, is the daemon running?\n");

      return 1;
    }

    printf("[nyafishzygisk]: command sent\n");
    ...
    struct nyafishzygisk_info info;
    nyafishzygiskd_get_info(&info);
    ...
    printf(
      ...
      " - trace <pid> [--restart]\n"
      " - ctl <start|stop|exit>\n"
      " - version: Shows the version of nyafishzygisk.\n"
