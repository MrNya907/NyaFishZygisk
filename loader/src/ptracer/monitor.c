...
struct nyafishzygiskd_status status64 = {
  .supported = false,
...
struct nyafishzygiskd_status status32 = {
  .supported = false,
  .zygote_injected = false,
  .daemon_running = false,
  .daemon_pid = -1,
...
bool nyafishzygiskd_listener_init() {
  ...
  size_t sun_path_len = sprintf(addr.sun_path, "%s/%s", nyafishzygiskd_get_path(), SOCKET_NAME);
...
void nyafishzygiskd_listener_callback() {
  while (1) {
    ...
    switch (msg.cmd) {
      ...
      case SYSTEM_SERVER_STARTED: {
        ...
        if (mount(PROP_PATH, "/data/adb/modules/nyafishzygisk/module.prop", NULL, MS_BIND, NULL) == -1) {
...
void nyafishzygiskd_listener_stop() {
...
static bool ensure_daemon_created(bool is_64bit) {
  struct nyafishzygiskd_status *status = is_64bit ? &status64 : &status32;

  if (is_64bit || (!is_64bit && !status64.supported)) {
    LOGD("new zygote started.");

    umount2("/data/adb/modules/nyafishzygisk/module.prop", MNT_DETACH);
  }
...
static bool prepare_environment() {
  ...
  FILE *orig_prop = fopen("/data/adb/modules/nyafishzygisk/module.prop", "r");
  if (orig_prop == NULL) {
  ...
  /* INFO: This environment variable is related to Magisk Zygisk/Manager. It
           ...
           Because of that behavior, we can knowledge built-in Zygisk is being
             used and stop the continuation of initialization of nyafishzygisk.*/
...
void init_monitor() {
  LOGI("nyafishzygisk %s", ZKSU_VERSION);
  ...
  struct monitor_event_cbs listener_cbs = {
    .callback = nyafishzygiskd_listener_callback,
    .stop_callback = nyafishzygiskd_listener_stop
  };
  if (!nyafishzygiskd_listener_init()) {
...
int send_control_command(enum nyafishzygiskd_command cmd) {
  ...
  size_t sun_path_len = snprintf(addr.sun_path, sizeof(addr.sun_path), "%s/%s", nyafishzygiskd_get_path(), SOCKET_NAME);
