...
#define ZYGISKD_FILE PATH_MODULES_DIR "/nyafishzygisk/bin/zygiskd" lp_select("32", "64")
#define ZYGISKD_PATH "/data/adb/modules/nyafishzygisk/bin/zygiskd" lp_select("32", "64")
...
static void load_modules(enum Architecture arch, struct Context *restrict context) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0 || strcmp(entry->d_name, "nyafishzygisk") == 0) continue;
